...
int Image::readFromFile(const char filename[]) {
    ...
    if (fentrada) {
        ...
        if (magic_n == "P2" || magic_n == "P5") {
            ...
            if (width >= 0 && height >= 0 && pixel_value == 255) {
...
bool Image::showImage(int k, Image &result) const{
    ...
    if(Byte::MIN_BIT<=k && k<=Byte::MAX_BIT-1){
    
        for (;pos<3;pos++)
            if (cod.at(pos) != (ShowByteAt(pos,k)).getValue()) salida=false;

        if (salida==true){

            int w=ShowByteAt(pos++,k).getValue();
            int h=ShowByteAt(pos++,k).getValue();
            if (_width*_height >= (w*h+5)*Byte::MAX_BIT){
...
bool Image::hideImage(int k, const Image &from){
    ...
    bool salida=true;
    const Byte cod[]={Byte('I'),Byte('M'), Byte('G')};
    int pos=0;
    int from_width=from._width;
    int from_height=from._height;
    int from_size=from_width*from_height;
    int n=(from_size+5)*8;
    
    if (n<=_width*_height && Byte::MIN_BIT<=k && k<=Byte::MAX_BIT-1){
        
        for (;pos<3;pos++) HideByteAt(cod[pos],pos,k);
        
        HideByteAt(Byte(from_width),pos++,k);
        HideByteAt(Byte(from_height),pos++,k);

        
        for (int i=0;i<from_size;i++) HideByteAt(from.getPos(i),pos++,k);
...
bool Image:: showText(char hidden [], int max, int k) const{
    ...
    if(Byte::MIN_BIT<=k && k<=Byte::MAX_BIT-1){
        ...
        if (salida==true){
            for (int i=0; i<max; i++){
                Byte aux = ShowByteAt(pos++,k);
                hidden[i]=aux.getValue();
            }

            if (ShowByteAt(pos,k).getValue() !=Byte(0).getValue() && pos <_width*_height) salida=false;
...
bool Image::hideText(const char source [], int k){
    ...
    if (Byte::MIN_BIT<=k && k<=Byte::MAX_BIT-1){
        ...
        if (_width*_height>=(longitud+4)*8){

            for (;pos<3;pos++) HideByteAt(cad[pos],pos,k);
            
            for (int i=0; i<=longitud; i++) HideByteAt(Byte(source[i]),pos++,k);
...
Byte Image::ShowByteAt(int i, int k) const{
    ...
    bool v_bits[Byte::NUM_BITS];
    Byte aux;
    
    for (int j=0; j<Byte::NUM_BITS; j++) v_bits[j]=Byte(getPos(i*Byte::NUM_BITS + j)).getBit(k);
   
    aux.encodeByte(v_bits);
    return aux;
    
}

void Image::HideByteAt(Byte byte, int i, int k){
    
    bool v_bits[Byte::MAX_BIT];
    byte.decodeByte(v_bits);
    
    for (int j=Byte::MAX_BIT-1; j>=0; j--){
        Byte aux(getPos(i*Byte::MAX_BIT+7-j));
        
        if (v_bits[j]) aux.onBit(k);
        else aux.offBit(k);
        
        setPos(i*Byte::MAX_BIT+7-j,aux);
    }
}
