...
62. Reescriba la solución del ejercicio 52 de esta Relación de Problemas usando
como estructura de datos para almacenar la tabla dentada una secuencia de
secuencias de datos de tipo RegistroDeMarca (secuencia de SecuenciaMarcas).

Escribir los métodos descritos en la figura 49.
...
private:
	...
	SecuenciaMarcas vector_privado[NUM_FILAS];

	int filas_utilizadas; //PRE: 0<=filas_utilizadas<=NUM_FILAS


	/**************************************************************************
	Metodo ReemplazaFila: que reemplaza una fila por otra.

	PRE: fila_a_borrar<filas_utilizadas && fila_a_borrar>=0
		 fila_origen<filas_utilizadas  && fila_origen>=0
	**************************************************************************/

	void ReemplazaFila (int fila_a_borrar, int fila_origen){
		if(fila_a_borrar<filas_utilizadas && fila_a_borrar>=0 &&
			fila_origen<filas_utilizadas  && fila_origen>=0)
			vector_privado[fila_a_borrar]=vector_privado[fila_origen];
	}
...
public:
	...
	//	   0<=columna<=vector_privado[fila].TotalUtilizados()
	...
	***************************************************************************/
	...
	void AniadeMarca(RegistroDeMarca m, int fila){
		int cols_fila=vector_privado[fila].TotalUtilizados();

		if(cols_fila<MAX_MARCAS_EN_PRUEBA){
			vector_privado[fila].Aniade(m);
		}
	}


	/**************************************************************************
	Metodo Inserta: introduce una fila nueva.
	...
	***************************************************************************/
	...
	void Inserta(SecuenciaMarcas fila_nueva, int fila_insercion){
		...
		if (num_cols_nueva <= MAX_MARCAS_EN_PRUEBA &&
			filas_utilizadas < NUM_FILAS &&
			fila_insercion >= 0 && fila_insercion <= filas_utilizadas) {
			...
			vector_privado[fila_insercion]=fila_nueva;
			filas_utilizadas++;
	...
	/**************************************************************************
	Metodo Elimina: que elimina una fila.
	...
	***************************************************************************/
	...
	void SustituyeFila(int fila, SecuenciaMarcas s){
		...
		if(numero_cols_fila_nueva<=MAX_MARCAS_EN_PRUEBA){
			vector_privado[fila]=s;
		}
	}
